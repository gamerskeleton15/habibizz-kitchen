// AnimatedGradient - a self-contained WebGL (WebGL2) animated gradient
// background. Renders a live canvas behind its children; everything inside
// sits on top. Accepts a preset ("Prism", "Lava", ...) or a "custom" config,
// an optional film-grain noise layer, and a corner radius.
//
// Ported to plain JSX from the shadcn/ui component (adapted for this project,
// which is React + Vite + Tailwind + JavaScript — no TypeScript).
// IMPORTANT fix: the original never rendered its children; this version does,
// so you can place text/UI over the gradient.

import { useRef, useEffect, useMemo, useState } from 'react'

// Numeric ids for the three mappable pattern shapes in the shader.
const PatternShapes = {
  Checks: 0,
  Stripes: 1,
  Edge: 2,
...
// Curated looks. Each is a full param set; you can override just `speed`
// on top of any preset by passing { preset: 'Prism', speed: 60 }.
const presets = {
  Prism: {
    color1: '#050505',
    color2: '#66B3FF',
...
// Every numeric field a preset carries — declared once so `resolveParams`
// always fills a complete, valid param object even for "custom" configs.
...
export default function AnimatedGradient({
  ...
  // Resolve whatever config we were given into a complete, well-formed
  // param set (all values present). Memoized so the animation loop's
  // effect only re-runs when the config actually changes.
  const params = useMemo(() => {
    if (config.preset === 'custom') {
      return {
        ...
        rotation: config.rotation ?? DEFAULT_PARAMS.rotation,
        proportion: config.proportion ?? DEFAULT_PARAMS.proportion,
        scale: config.scale ?? DEFAULT_PARAMS.scale,
        speed: config.speed ?? DEFAULT_PARAMS.speed,
        distortion: config.distortion ?? DEFAULT_PARAMS.distortion,
        swirl: config.swirl ?? DEFAULT_PARAMS.swirl,
        swirlIterations: config.swirlIterations ?? DEFAULT_PARAMS.swirlIterations,
        softness: config.softness ?? DEFAULT_PARAMS.softness,
        offset: config.offset ?? DEFAULT_PARAMS.offset,
        shape: config.shape ?? DEFAULT_PARAMS.shape,
        shapeSize: config.shapeSize ?? DEFAULT_PARAMS.shapeSize,
      }
    }
  ...
  // Set up WebGL2, compile the shaders, and run the animation loop.
...
// ---- GLSL fragment shader (unchanged) ----
